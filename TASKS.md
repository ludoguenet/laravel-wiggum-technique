...
Re-read this file every pass. Work on exactly **one uncheck task** per task, then check it off below this checklist, not just a green test run, is what tells the loop whether to stop.

## Progress checklist

- [x] Feature 1 - Rename team
- [x] Feature 2 - Leave team
- [x] Feature 3 - Cancel pending invitation
...
- A non-owner attempting to rename gets a 403 (dealt with `TeamPolicy@update`)
- Renaming does not change `owner_id`, members, or invitations (pending or not)
- Validation : `name` required, string, same rules as team creation. Do not invent new rules.

**Acceptance test:** owner can rename -> team persists new name. Non-owner attempts to rename -> 403, name unchangged.
